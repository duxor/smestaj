...
//TEMA START::
	public function getTema(){
		...
		return Security::autentifikacija('administracija.aplikacija.tema.index', compact('teme'),5,'min');
	}
	public function getTemaNova(){
		return Security::autentifikacija('administracija.aplikacija.tema.edit', null,5,'min');
	}
...
//TEMPLEJT START::
	public function getTemaTemplejt($slug){
		...
		return Security::autentifikacija('administracija.aplikacija.templejt.index',compact('templejt'),5,'min');
	}
	...
	public function postTemplejtEdit($slug){
		...
		return Security::autentifikacija('administracija.aplikacija.templejt.edit',compact('templejt','vrstaSadrzaja','tema_slug'),5,'min');
	}
	public function postTemplejtNovi(){
		$vrstaSadrzaja=VrstaSadrzaja::orderBy('id')->get(['id','naziv'])->lists('naziv','id');
		$templejt=['tema_id'=>Input::get('tema_id'), 'tema_slug'=>Input::get('tema_slug')];
		return Security::autentifikacija('administracija.aplikacija.templejt.edit',compact('vrstaSadrzaja','templejt'),5,'min');
	}
